    ...
    public class LinkDataReader
    {
        ...
        static List<int> GetListInt(ISheet sheet, int rowIndex, int colIndex)
        {
            ...
            for (int i = sheet.FirstRowNum + rowIndex; i <= sheet.LastRowNum; ++i)
            {
                ...
                list.Add(ReadHelper.GetIntValue(cell));
            }
            return list;
        }
        ...
        static List<long> GetListLong(ISheet sheet, int rowIndex, int colIndex)
        {
            ...
            for (int i = sheet.FirstRowNum + rowIndex; i <= sheet.LastRowNum; ++i)
            {
                IRow row = sheet.GetRow(i);
                ICell cell = row.GetCell(row.FirstCellNum + colIndex);
                list.Add(ReadHelper.GetLongValue(cell));
            }
            return list;
        }
        ...
        static List<float> GetListFloat(ISheet sheet, int rowIndex, int colIndex)
        {
            ...
            for (int i = sheet.FirstRowNum + rowIndex; i <= sheet.LastRowNum; ++i)
            {
                IRow row = sheet.GetRow(i);
                ICell cell = row.GetCell(row.FirstCellNum + colIndex);
                list.Add(ReadHelper.GetFloatValue(cell));
            }
            return list;
        }
        ...
        static List<double> GetListDouble(ISheet sheet, int rowIndex, int colIndex)
        {
            ...
            for (int i = sheet.FirstRowNum + rowIndex; i <= sheet.LastRowNum; ++i)
            {
                IRow row = sheet.GetRow(i);
                ICell cell = row.GetCell(row.FirstCellNum + colIndex);
                list.Add(ReadHelper.GetDoubleValue(cell));
            }
            return list;
        }
        ...
        static List<string> GetListString(ISheet sheet, int rowIndex, int colIndex)
        {
            ...
            for (int i = sheet.FirstRowNum + rowIndex; i <= sheet.LastRowNum; ++i)
            {
                IRow row = sheet.GetRow(i);
                ICell cell = row.GetCell(row.FirstCellNum + colIndex);
                list.Add(ReadHelper.GetStringValue(cell));
            }
            return list;
        }
        ...
        static List<bool> GetListBool(ISheet sheet, int rowIndex, int colIndex)
        {
            ...
            for (int i = sheet.FirstRowNum + rowIndex; i <= sheet.LastRowNum; ++i)
            {
                IRow row = sheet.GetRow(i);
                ICell cell = row.GetCell(row.FirstCellNum + colIndex);
                list.Add(ReadHelper.GetBoolValue(cell));
            }
            return list;
        }
        ...
        static object GetArrayData(ISheet sheet, int rowIndex, int colIndex, Type t)
        {
            ...
            {
                Schema schema = SchemaReader.ReadSchema(sheet);
                return DataReader.ReadList(sheet, schema).ToArray();
            }
            else
            {
                return null;
        ...
        public static object GetLinkDict(ICell cell, string keyField,bool removeKeyFieldInElement=false)
        {
            ...
            //内容要跳过头
            return DataReader.ReadDictionary(linkSheet, schema, keyField,cp.row+EDConstance.SchemaDataRow,cp.col,null, removeKeyFieldInElement);
        }
    }
}
